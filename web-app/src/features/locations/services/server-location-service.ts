...
class ServerLocationService implements LocationService {
  ...
  async searchPlaces(query: string): Promise<PlaceSearchResult[]> {
    try {
      const searchQuery = `${query} cafe coffee shop`;
      ...
      const searchResults: PlaceSearchResult[] = data.results
        .filter((place: any) => place.place_id && place.name && place.formatted_address)
        .map((place: any) => ({
  ...
  async getPlaceDetails(placeId: string): Promise<CafeInfo> {
    try {
      ...
      if (data.status !== "OK") {
        console.error("Place details request failed:", data.status, data.error_message);
